...
describe Book do
  context '#search' do
    it 'should give empty array when no book present with title having search parameter' do
      FactoryGirl.create(:book, title:'Malgudi days')
      FactoryGirl.create(:book, title:'Java programming')
      expect(Book.search('the')).to eq([])
    end

    it 'should give book with title having search parameter' do
      book1 = FactoryGirl.create(:book, title:'Malgudi days')
      book2 = FactoryGirl.create(:book, title:'Java programming')
      book3 = FactoryGirl.create(:book, title:'Ruby programming')
